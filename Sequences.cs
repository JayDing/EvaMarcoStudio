...
public class SequenceForm:Form {
 ...
 SequenceItem CloneItem(SequenceItem item){return json.Deserialize<SequenceItem>(json.Serialize(item));}
...
public static class SequenceSession {
 ...
 static JavaScriptSerializer Serializer(){return new JavaScriptSerializer{MaxJsonLength=64*1024*1024};}
 public static SequencePlan Load(string path){if(!File.Exists(path))return new SequencePlan();var plan=Serializer().Deserialize<SequencePlan>(File.ReadAllText(path));SequencePlan.Validate(plan);return plan;}
 public static void Save(string path,SequencePlan plan){
  SequencePlan.Validate(plan);var full=Path.GetFullPath(path);Directory.CreateDirectory(Path.GetDirectoryName(full));string temp=full+"."+Guid.NewGuid().ToString("N")+".tmp";
  try{File.WriteAllText(temp,Serializer().Serialize(plan),System.Text.Encoding.UTF8);if(File.Exists(full))File.Replace(temp,full,null);else File.Move(temp,full);}finally{if(File.Exists(temp))File.Delete(temp);}
 }
}