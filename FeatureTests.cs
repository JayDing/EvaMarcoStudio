...
public static class FeatureTests {
 ...
 public static void Run(){TestTemplateSaving();
  ...
  using(var picker=new KeyPicker()){Check(picker.Value=="Space","Single-key default");picker.Value="Ctrl+C";foreach(Control c in picker.Controls)if(c is Button&&c.Text=="Enter")((Button)c).PerformClick();Check(picker.Value=="Enter","Common button replaces combination with single key");
  ...
  using(var main=new MainForm()){var nums=Descendants(main).OfType<NumericUpDown>();Check(nums.Count(n=>n.Maximum==86400000&&n.Value==1000)==1&&nums.Count(n=>n.Maximum==86400&&n.Value==1)==1,"1000ms default delay and gap");Check(Descendants(main).OfType<CrosshairDrag>().Count()==1,"New mouse action has draggable crosshair");}
 ...
 static void TestLiveUI(){
  ...
  using(var badge=new RunBadge())using(var bitmap=new Bitmap(badge.Width,badge.Height)){badge.SetProgress("A 範本",999);badge.DrawToBitmap(bitmap,new Rectangle(Point.Empty,badge.Size));bitmap.Save(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"preview-running.png"));}
